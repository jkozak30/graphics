...
def calculate_diffuse(light, reflect, normal):
    ...
    while LOC < len(light):
        dot = dot_product( light[LOC], normal)
        ...
        # print(COL)
        # print(light[COL])
        t[RED] = light[COL][RED] * reflect['red'][DIFFUSE] * dot
        t[GREEN] = light[COL][GREEN] * reflect['green'][DIFFUSE] * dot
        t[BLUE] = light[COL][BLUE] * reflect['blue'][DIFFUSE] * dot
        limit_color(t)
        d[RED] += t[RED]
        d[GREEN] += t[GREEN]
        d[BLUE] += t[BLUE]
...
def calculate_specular(light, reflect, view, normal):
    ...
    while LOC < len(light):
        result = 2 * dot_product(light[LOC], normal)
        n[0] = (normal[0] * result) - light[LOC][0]
        n[1] = (normal[1] * result) - light[LOC][1]
        n[2] = (normal[2] * result) - light[LOC][2]

        result = dot_product(n, view)
        result = result if result > 0 else 0
        result = pow( result, SPECULAR_EXP )

        t[RED] = light[COL][RED] * reflect['red'][SPECULAR] * result
        t[GREEN] = light[COL][GREEN] * reflect['green'][SPECULAR] * result
        t[BLUE] = light[COL][BLUE] * reflect['blue'][SPECULAR] * result
        s[RED] += t[RED]
        s[GREEN] += t[GREEN]
        s[BLUE] += t[BLUE]
        LOC += 2
