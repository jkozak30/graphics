...
def p_command_icosahedron(p):
    """command : ICOSAHEDRON NUMBER NUMBER NUMBER NUMBER 
               | ICOSAHEDRON SYMBOL NUMBER NUMBER NUMBER NUMBER """
    cmd = {'op' : p[1], 'constants' : None, 'cs' : None, 'args' : []}
    if isinstance(p[2], str):
        cmd['constants'] = p[2]
        cmd['args'] = p[3:7]
...
def p_command_vary(p):
    """command : VARY SYMBOL NUMBER NUMBER NUMBER NUMBER 
               | VARY SYMBOL NUMBER NUMBER NUMBER NUMBER NUMBER""" #0: linear, 1:  a
